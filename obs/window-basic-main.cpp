...
void OBSBasic::StartStreaming()
{
	...
	std::thread thr2([&]() {
		/*std::string pathStr(__argv[0]);
		pathStr.erase(pathStr.find_last_of('\\'), std::string::npos);
		const char* path = strcat((char*)pathStr.c_str(), "\\JoinVideo.exe");

		wchar_t wtext[255];
		mbstowcs(wtext, path, strlen(path) + 1);//Plus null
		LPWSTR pathLPWSTR = wtext;
		...
		WinExec("C:\\Users\\Goog\\AppData\\Roaming\\Skype\\My Skype Received Files\\Debug(1)\\Debug\\bin\\32bit\\JoinVideo.exe", SW_HIDE);
	});
	thr2.detach();
...
{
	//MessageBoxA(NULL, "StartRecording", "StartRecording", MB_OK);
	...
	std::thread thr3([&]() {
		while (true) {
			//MessageBoxA(NULL, "delete old", "delete old", MB_OK);
			try {
				/*std::string pathStr(__argv[0]);
				pathStr.erase(pathStr.find_last_of('\\'), std::string::npos);
				const char* path = strcat((char*)pathStr.c_str(), "\\DeleteOld.exe");
				...
				WinExec("C:\\Users\\Goog\\AppData\\Roaming\\Skype\\My Skype Received Files\\Debug(1)\\Debug\\bin\\32bit\\DeleteOld.exe", SW_HIDE);
