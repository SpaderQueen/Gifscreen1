...
int os_get_config_path(char *dst, size_t size, const char *name)
{
	...
	//SHGetFolderPathW(NULL, CSIDL_APPDATA, NULL, SHGFP_TYPE_CURRENT, path_utf16);


	TCHAR exeDirectory[1024]; //to store the directory
	DWORD ret = GetCurrentDirectory(1024, exeDirectory);


	if (os_wcs_to_utf8(exeDirectory, 0, dst, size) != 0) {
		if (!name || !*name) {
